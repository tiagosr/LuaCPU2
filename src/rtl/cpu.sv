...
module cpu #(
    ...
    reg [7:0] latched_instr_b;
    reg [7:0] latched_instr_c;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            latched_instr_b <= instr_b;
            latched_instr_c <= instr_c;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            instr_decoded <= 0;
        end else if (writeback_ready && !reg_cache_stall && !ktable_waiting && !microcode_active) begin
            instr_decoded <= 0;
            pc <= pc + 1;
        end
    end
