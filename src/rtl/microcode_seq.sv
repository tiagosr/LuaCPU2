...
module microcode_seq #(
    ...
    output reg [1:0] micro_active,
    output reg [1:0] micro_done,
    output reg [1:0] micro_writeback_ready,
    output reg [9:0] branch_target,
    output reg [10:0] sequencer_addr_out,
    input  wire [1:0] micro_stall_in
);
    ...
    always @(*) begin
        case (opcode_key)
            7'h01: immediate_source = {26'h0000000000, instr_c};
            7'h02: immediate_source = {17'h00000, instr_bx};
            7'h03: immediate_source = {26'h0000000000, instr_b};
            7'h04: immediate_source = {9'h0, instr_ax};
            7'h0E: immediate_source = {26'h0000000000, instr_c};
            7'h0F: immediate_source = {26'h0000000000, instr_c};
            7'h10: immediate_source = {26'h0000000000, instr_c};
            7'h00: immediate_source = 34'h0;
    ...
    always @(*) begin
        if (reset) begin
            ...
            multi_step_active_comb = 1'b0;
            next_micro_writeback_ready = 1'b0;
        end else begin
            case (micro_active)
                1'b0: begin
                    if (instr_decoded) begin
                        ...
                        if (rom_micro_branch == 2'h0 && (opcode_key == 7'h00 || opcode_key == 7'h0E || opcode_key == 7'h0F || opcode_key == 7'h10)) begin
                            multi_step_active_comb = 1'b1;
                        end else begin
                            multi_step_active_comb = 1'b0;
                        end
                    end else begin
                ...
                1'b1: begin
                    ...
                    if (micro_stall_in) begin
                        next_internal_addr = sequencer_addr_out;
                        next_micro_active = micro_active;
                        next_micro_done = micro_done;
                        multi_step_active_comb = multi_step_active_comb;
                    end else begin
                        case (rom_micro_branch)
                            2'h0: begin
                                if (multi_step_active_comb) begin
                                    next_internal_addr = sequencer_addr_out + 1;
                                    next_micro_active = micro_active;
                                    next_micro_done = 1'b0;
                                end else begin
                                    next_internal_addr = sequencer_addr_out + DONE_OFFSET;
                                    next_micro_active = micro_active;
                                    next_micro_done = 1'b0;
                                end
                            end
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            micro_active <= 0;
            micro_done <= 1;
            micro_writeback_ready <= 0;
            branch_target <= 10'h0;
            next_multi_step_active <= 0;
        end else begin
            rom_address <= next_internal_addr;
            sequencer_addr_out <= next_internal_addr;
            alu_op <= next_alu_op;
            ...
            next_multi_step_active <= multi_step_active_comb;
        end
    end
